...
const PRESENTATION_LOCALES = Object.freeze({
  ko: {
    ...
    generated: {
      ...
      reaction: '반응',
      success: '성공',
      day: (index) => `${index + 1}일`, experiment: (index) => `실험 ${index + 1}`,
      remember: '기억할 말',
      card: '곁에 두는 양육 카드',
      ideas: '선택적 참고 아이디어',
      close: '방법과 한계',
...
const LATIN_LOCALE_OVERRIDES = Object.freeze({
  ...
  id: {
    ...
    generated: { focus: 'Fokus pengasuhan bulan ini', scene: (i) => `Adegan salah paham ${i + 1}`, signature: (i) => `Tanda khas perilaku ${i + 1}`, inferencePrefix: 'Perkiraan', script: (i) => `Skrip percakapan ${i + 1}`, referencePrefix: 'Referensi', careerHint: 'Petunjuk arah karier', flow: 'Alur referensi periode ini', avoid: 'Yang perlu dihindari', readingMethod: 'Cara membaca', readingMethodText: 'Ini bukan ramalan yang menetapkan masa depan, melainkan garis referensi untuk observasi.', smallExperiment: 'Eksperimen kecil', reaction: 'Respons', success: 'Keberhasilan', day: (i) => `Hari ${i + 1}`, experiment: (i) => `Eksperimen ${i + 1}`, remember: 'Kata untuk diingat', card: 'Kartu pengasuhan untuk disimpan', ideas: 'Ide referensi opsional', close: 'Metode dan batasan' },
  },
  es: {
  ...
  try {
    ...
    const presentation = {
      ...
      sections: [
        ...
        { number: 7, title: sections[6].title, blocks: withIntro(7, [{ type: 'checklist', title: generated.smallExperiment, items: s7.map((g, i) => ({ label: (s7.length >= 6 ? generated.day : (generated.experiment || generated.day))(i), text: `${g[label.s7[0]]} / ${generated.reaction}: ${g[label.s7[1]]} / ${generated.success}: ${g[label.s7[2]]}` })) }]) },
