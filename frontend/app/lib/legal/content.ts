...
const privacyContent: Record<SupportedLegalLanguage, LegalPageContent> = {
    ...
    sections: [
      {
        ...
        table: {
          ...
          rows: [
            ['Free preview', 'Child name, birth date/time, gender, birth place, parent role and parent birth date/time', 'Generate a temperament preview', 'Not stored in the database. Temporary infrastructure/API logs may be retained by service providers'],
  ...
  ja: {
    ...
    sections: [
      { title: '1. 取り扱う個人データ', table: { headers: ['場面', 'データ', '目的', '保管'], rows: [['無料プレビュー', 'お子様の名前、生年月日、出生時刻、性別、出生地、保護者の役割・生年月日・出生時刻', '気質プレビューの生成', 'データベースには保存しません。ただしインフラ/APIログは一時的に残る場合があります'], ['有料/プロモレポート', '入力情報、メール、生成されたSajuデータ、AIレポート、PDF', 'レポート生成、表示、送付、サポート', '限定的なアクセスとサポートのため保管。削除依頼時は法令上必要な記録を除き削除'], ['決済', 'メール、PayPal注文/承認情報、決済状態、金額', '決済、返金、取引記録', '商取引・税務・紛争対応に必要な期間'], ['分析Cookie', 'Google Analytics Cookie/識別子', 'サービス改善分析', '同意した場合のみ使用']] } },
